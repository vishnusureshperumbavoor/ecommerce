...
var instance = new Razorpay({
    key_id: 'rzp_test_wAPQNdTltKIe1F',
    key_secret: 'nrtDNkllfhTBPGs8h9hQ6zbX',
});
...
module.exports = {
    ...
    doLogin : (userData)=>{
        return new Promise(async(resolve,reject)=>{
            let response = { }
            let customer = await db.get().collection(collection.USER_COLLECTION).findOne({email:userData.email})
            if(customer){
    ...
    changePaymentStatus:(receipt)=>{
        return new Promise((resolve,reject)=>{
            db.get().collection(collection.ORDER_COLLECTION).updateOne({_id:objectId(receipt)},
            {
                $set:{
                    status : 'placed'
                }
    ...
    placedOrders:(userId)=>{
        return new Promise(async(resolve,reject)=>{
            let orders = await db.get().collection(collection.ORDER_COLLECTION).find({
                $and:
                    [
                        {userId:objectId(userId)},{"status":"placed"}
                    ]
            }).toArray()
            resolve(orders)
        })
    ...
    pendingOrders:(userId)=>{
        return new Promise(async(resolve,reject)=>{
            let orders = await db.get().collection(collection.ORDER_COLLECTION).find({
                $and:
                    [
                        {userId:objectId(userId)},{"status":"pending"}
                    ]
            }).toArray()
            resolve(orders)
        })
    ...
    shippedOrders:(userId)=>{
        return new Promise(async(resolve,reject)=>{
            let orders = await db.get().collection(collection.ORDER_COLLECTION).find({
                $and:
                    [
                        {userId:objectId(userId)},{"status":"shipped"}
                    ]
            }).toArray()
            resolve(orders)
        })
    ...
    pendingCOD:(orderId)=>{
        console.log("entered isnide poendingcod orderid");
        return new Promise((resolve,reject)=>{
